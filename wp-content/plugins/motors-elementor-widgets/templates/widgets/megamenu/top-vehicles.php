		...
		<?php if ( $query->have_posts() ) : ?>
			<ul class="top-vehicles">
				...
				while ( $query->have_posts() ) :
					...
					<li>
						<a class="normal_font"
								href="<?php echo esc_url( get_the_permalink( stm_listings_user_defined_filter_page() ) ) . '?make=' . esc_attr( $make ) . '&serie=' . esc_attr( $serie ); ?>"><?php echo esc_html( $make_name->name . ' ' . $model_name->name ); ?></a>
					</li>
				<?php endwhile; ?>
			</ul>
			<?php

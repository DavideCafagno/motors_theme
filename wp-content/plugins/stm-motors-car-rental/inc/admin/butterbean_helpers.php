...
add_action('plugins_loaded', 'stm_motors_car_rental_load_butterbean');
function stm_motors_car_rental_load_butterbean()
{
    require_once(STM_MOTORS_CAR_RENTAL_PATH . '/inc/admin/butterbean/butterbean.php');
}

function stm_motors_car_rental_validate_checkbox($value)
{
    return wp_validate_boolean($value) ? 'on' : false;
}

function stm_motors_car_rental_no_validate($value)
{
    return $value;
}

function stm_motors_car_rental_validate_image($value)
{
    return !empty($value) ? intval($value) : false;
}

function stm_motors_car_rental_validate_repeater($value, $butterbean)
{
...
function stm_motors_car_rental_multiselect($value, $butterbean)
{
...
function stm_motors_car_rental_validate_gallery($value)
{
    $value = explode(',', $value);
    $values = array();
...
function stm_motors_car_rental_date($value) {
	return strtotime($value);
}

function stm_motors_car_rental_start_timestamp($value) {
	global $post;
	$post_id = $post->ID;
	$data = $_POST;
...
function stm_motors_car_rental_get_user_list()
{
    $users_args = array(
    ...
    $users_dropdown = array(
        '' => esc_html__('Not assigned', 'stm_motors_car_rental')
    );
...
function stm_motors_car_rental_add_category_in()
{
    $response = array();
    $category = $term = '';
    ...
    if (!empty($term) and !empty($category)) {
        ...
        if ($term_id === 0 or $term_id === null) {
            ...
            $response['message'] = sprintf(esc_html__('%s already added!', 'stm_motors_car_rental'), $term_info->name);
        ...
        if (!empty($term_id) and !is_wp_error($term_id)) {
            $term_info = get_term_by('id', $term_id['term_id'], $category);
            $response['message'] = sprintf(esc_html__('%s added!', 'stm_motors_car_rental'), $term_info->name);
            $response['status'] = 'success';
            $response['slug'] = $term_info->slug;
            $response['name'] = $term_info->name;
        } else {
...
add_action('wp_ajax_stm_motors_car_rental_add_category_in', 'stm_motors_car_rental_add_category_in');
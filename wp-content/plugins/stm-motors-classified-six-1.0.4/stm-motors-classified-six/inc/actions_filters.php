<?php
add_filter( 'stm_hide_old_headers', 'stm_c_six_theme_hide_old_headers' );
add_filter( 'stm_is_ulisting_layout', 'stm_is_c_six_ulisting_layout' );
...
add_filter( 'stm_get_query_vars', 'stm_c_six_get_query_vars');
add_filter( 'ulisting_account_endpoint', 'stm_c_six_ulisting_account_endpoint');
...
add_action( 'stm_motors_header', 'stm_c_six_include_header' );
add_action( 'ulisting_profile_edit', 'stm_c_six_ulisting_profile_edit', 100, 1);

if ( !function_exists( 'stm_c_six_theme_hide_old_headers' ) ) {
	function stm_c_six_theme_hide_old_headers()
	{
		return false;
	}
}

if ( !function_exists( 'stm_c_six_include_header' ) ) {
	function stm_c_six_include_header ()
	{
		if ( !is_404() and !is_page_template( 'coming-soon.php' ) ) {
			stm_c_six_load_template( 'header/header' );
		} elseif ( is_page_template( 'coming-soon.php' ) ) {
			stm_c_six_load_template( 'header/header-coming', 'soon' );
		} else {
			stm_c_six_load_template( 'header/header', '404' );
		}
...
if(!function_exists('stm_is_c_six_ulisting_layout')) {
	function stm_is_c_six_ulisting_layout() {
		return true;
	}
}
...
if(!function_exists('stm_ulisting_inventory_layout_data')) {
	function stm_ulisting_inventory_layout_data ($data) {
		...
		$data['elements'][] = [
			...
			"field_group"  => "general",
			"params"       => [
				"template_path"		=> "listing-list/ulisting-sidebar",
				"template"          => "template_1",
				"type"              => "found_posts",
				"id"                => "",
				"class"             => "",
...
if(!function_exists('stm_c_six_get_query_vars')) {
	function stm_c_six_get_query_vars ($vars) {
		return $vars;
	}
}

if(!function_exists('stm_c_six_ulisting_account_endpoint')) {
	function stm_c_six_ulisting_account_endpoint($vars) {
		return $vars;
	}
}
...
if(!function_exists('stm_ulisting_attribute_gallery_style_templates')) {
	function stm_ulisting_attribute_gallery_style_templates( $styles )
	{
		...
		$styles["ulisting_gallery_style_1"] = [ "icon" => STM_MOTORS_C_SIX_URL . "/assets/img/ulisting/builder/gallery/gallery_style_1.jpg", "name" => "Style 1", ];
...
add_filter("ulisting_attribute_style_templates", function($styles) {
	...
	$styles["ulisting_style_4"] = [ "icon" => STM_MOTORS_C_SIX_URL . "/assets/img/ulisting/builder/attributes/attribute_style_4.jpg", "name" => "Style 4", "attribute_template" => "
	...
	$styles["ulisting_style_5"] = [ "icon" => STM_MOTORS_C_SIX_URL . "/assets/img/ulisting/builder/attributes/attribute_style_5.jpg", "name" => "Style 5", "attribute_template" => "
	...
	$styles["ulisting_style_6"] = [
		"icon" => STM_MOTORS_C_SIX_URL . "/assets/img/ulisting/builder/attributes/attribute_style_6.jpg",
		"name" => "Style 6",
		"attribute_template" => "
...
function stm_ajax_get_c_six_user_phone()
{
	check_ajax_referer( 'stm_ajax_get_c_six_user_phone', 'security' );
...
add_action( 'wp_ajax_stm_ajax_get_c_six_user_phone', 'stm_ajax_get_c_six_user_phone' );
add_action( 'wp_ajax_nopriv_stm_ajax_get_c_six_user_phone', 'stm_ajax_get_c_six_user_phone' );
...
stm_c_six_module_enqueue_scripts_styles('stm_browse_by_type');

$atts = stm_c_six_get_content_tags($content);
...
<div class="stm-c-six-categories-tabs-wrap <?php echo esc_attr( $css_class ); ?>">
	<h2>
		<?php echo esc_html($categories_title);?>
	</h2>

	<div class="stm-c-six-tabs-wrap">
		<ul class="nav nav-tabs" id="myTab" role="tablist">
            <?php if(!empty($show_all_tab) && $show_all_tab == 'yes') : ?>
            <li class="nav-item">
                <a class="nav-link active" id="all-<?php echo esc_attr($rand); ?>-tab" data-toggle="tab" href="#all-<?php echo esc_attr($rand); ?>" role="tab" aria-controls="all-<?php echo esc_attr($rand); ?>" aria-selected="true"><?php echo esc_html__('All', 'stm_motors_classified_six');?></a>
            </li>
            <?php endif; ?>
			<?php foreach ($scAtts as $k => $cat) : $id = 'id-' . $cat['listing_category']; ?>
		...
		<div class="tab-content" id="myTabContent">
            ...
            <div class="tab-pane fade show active" id="all-<?php echo esc_attr($rand); ?>" role="tabpanel" aria-labelledby="all-<?php echo esc_attr($rand); ?>-tab">
				...
				uListing\Classes\StmListingTemplate::load_template(
					...
					[
						...
						"view_type" => 'grid',
						"item_class" => 'stm-c-six-by-type-item',
					], true);
				?>
            </div>
            <?php endif; ?>
            ...
            foreach ($scAtts as $k => $cat) : $id = 'id-' . $cat['listing_category']; ?>
				<div class="tab-pane fade <?php echo (empty($show_all_tab) && $k == 0) ? 'show active' : ''; ?>" id="<?php echo esc_attr($id); ?>" role="tabpanel" aria-labelledby="<?php echo esc_attr($id); ?>-tab">
					...
					<?php if(!empty($load_more) && $load_more == 'yes'):
                        ...
                        <div class="stm-load-more">
                            <a id="<?php echo esc_attr($cat['listing_category']); ?>-l-m-<?php echo esc_attr($rand); ?>" href="<?php echo (!empty($page)) ? get_the_permalink($page->ID) : '#'; ?>" class="button"><?php echo esc_html__('Show More', 'stm_motors_classified_six');?></a>
                        </div>
					<?php endif; ?>
				</div>
			<?php endforeach;?>

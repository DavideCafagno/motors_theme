<?php
function stm_c_six_get_icons_html()
{
	$fa_icons = stm_get_c_six_icons_fa('fa');
	$custom_icons = stm_get_c_six_icons('stm-all-icon');
...
function stm_get_c_six_icons($font_pack = 'fa')
{
	$fonts = array();

	if(file_exists(STM_MOTORS_C_SIX_PATH . '/assets/fonts/font-json/' . $font_pack . '.json')) {
		$font_icons = file( STM_MOTORS_C_SIX_PATH . '/assets/fonts/font-json/' . $font_pack . '.json' );
...
function stm_get_array_icons($font_pack)
{
	...
	if(file_exists(STM_MOTORS_C_SIX_PATH . '/assets/fonts/font-json/stm-all-icon.json')) {
		$font_icons = file( STM_MOTORS_C_SIX_PATH . '/assets/fonts/font-json/stm-all-icon.json' );
		$font_icons = json_decode( implode( '', $font_icons ), true );
...
function stm_get_c_six_icons_fa()
{
	$fonts = array();

    $font_icons = file( STM_MOTORS_C_SIX_PATH . '/assets/fonts/font-json/fa.json' );
    $font_icons = json_decode( implode( '', $font_icons ), true );
...
function stm_c_six_get_category_icon($termId) {
	$icon = get_term_meta($termId, 'stm_c_six_icon', true);

	return $icon;
}

function stm_c_six_get_content_tags($content)
{
...
function stm_c_six_ulisting_html_attributes($data) {
    return $data;
}

//add_filter('ulisting_html_attributes', 'stm_c_six_ulisting_html_attributes');
 ...
 *
 * */
function stm_c_six_get_page_url ($page) {
...
if ( !function_exists( 'stm_c_six_display_user_name' ) ) {
	/**
	 * User display name
	 *
	 * @param $user_id
	 ...
	 * @param string $l_name
	 */
	function stm_c_six_display_user_name( $user_id, $user_login = '', $f_name = '', $l_name = '' )
	{
